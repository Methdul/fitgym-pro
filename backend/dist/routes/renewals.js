...
exports.renewalRoutes = void 0;
// backend/src/routes/renewals.ts
const express_1 = __importDefault(require("express"));
const supabase_1 = require("../lib/supabase");
const auth_1 = require("../middleware/auth");
const router = express_1.default.Router();
exports.renewalRoutes = router;
// Process a member renewal (uses Edge Function when available, fallback to direct DB)
router.post('/process', async (req, res) => {
    try {
        const { memberId, packageId, paymentMethod, amountPaid, durationMonths, staffId, staffPin } = req.body;
        ...
        try {
            ...
            const { data: staff, error: staffError } = await supabase_1.supabase
                .from('branch_staff')
                .select('*')
                .eq('id', staffId)
                .single();
            ...
            const { data: member, error: memberError } = await supabase_1.supabase
                .from('members')
                .select('*')
                .eq('id', memberId)
                .single();
            ...
            const { data: renewal, error: renewalError } = await supabase_1.supabase
            ...
            const { error: memberUpdateError } = await supabase_1.supabase
                .from('members')
                .update({
                status: 'active',
...
router.get('/member/:memberId', auth_1.optionalAuth, async (req, res) => {
    try {
...
router.get('/recent/:branchId', auth_1.optionalAuth, async (req, res) => {
    try {
...
router.get('/eligibility/:memberId', auth_1.optionalAuth, async (req, res) => {
    try {
        const { memberId } = req.params;
