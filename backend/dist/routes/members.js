...
const generateMemberEmail = (nationalId) => {
...
router.post('/', async (req, res) => {
    try {
        ...
        const { branch_id, first_name, last_name, email, phone, national_id, status = 'active', package_type = 'individual', package_name, package_price, start_date, expiry_date, is_verified = false, is_existing_member = false // New flag to distinguish member types
         } = req.body;
        // Validation
        ...
        const { data: existingNationalId } = await supabase_1.supabase
            .from('members')
        ...
        const userAccountData = await createUserAccount(accountEmail, national_id, // Use national_id as temporary password
        {
            first_name,
            last_name,
        ...
        const { data: memberData, error: memberError } = await supabase_1.supabase
            .from('members')
            .insert({
        ...
        if (memberError) {
            ...
            try {
                await supabase_1.supabase.from('users').delete().eq('id', userAccountData.userProfile.id);
                await supabase_1.supabase.auth.admin.deleteUser(userAccountData.authUser.id);
...
router.put('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Get existing member
        const { data: existingMember, error: fetchError } = await supabase_1.supabase
            .from('members')
            .select('*')
            .eq('id', id)
            .single();
...
router.delete('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { data: member, error: fetchError } = await supabase_1.supabase
            .from('members')
            .select('*, branches(name)')
            .eq('id', id)
            .single();
...
// Search members
router.get('/search/:branchId', auth_1.optionalAuth, async (req, res) => {
    try {
